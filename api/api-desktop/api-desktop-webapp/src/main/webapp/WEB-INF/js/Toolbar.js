...
init:function () {
  ...
  if (this.border != 'none') {
    this.toolbar.css('border-' + this.border, 'solid 2px #8DB2E3');
  }
  ...
  if (it) {
    if (dis) {
	  ...
	  //--设置不可以用的时候，同时设置掉他的方法为false
	  it.button.unbind('click', it.handler);
    } else {
...
$(function(){
   //$(document.body).append("<div id='toolbar'></div>");
	InitToolBar();
})
...
function InitToolBar(){
   var toolbar = new Toolbar({
        ...
        items : [{
          type : 'button',
          text : '新增',
          bodyStyle : 'new_toolbar',
          useable : 'T',
          handler :doNew
        },'-',{
          type : 'button',
          text : '修改',
          bodyStyle : 'edit_toolbar',
          useable : 'T',
          handler : doEdit
        },'-',
        {
        ...
        '-',			
        {
          type : 'button',
          text : '导入',
          bodyStyle : 'import_toolbar',
          useable : 'T',
          handler :doImp
        },'-',{
          type : 'button',
          text : '导出EXCEL',
          bodyStyle : 'export_toolbar',
          useable : 'T',
          handler :doExport
        }]
      });
	  toolbar.render();
	   ...
	   //alert('Selected: '+tab.panel('options').bMap);
	  if(tab){
	     setToolbarButton(toolbar,tab.panel('options').bMap);
	  }
	  //alert(top.tabpanel.getActiveTab().btnMap);
	  				//var tab = top.tabpanel.tabs('getSelected');
			//alert(tab.panel('options').title+"######"+tab.panel('options').tabId);
		 	tmsJ5ModuleTitle=tab.panel('options').title;
  			tmsJ5ModuleId=tab.panel('options').tabId;

}

//0-1 增加 1-2 修改  2-3 删除 3-4 导入  4-5 导出到excel  5-6 1  6-7 1 7-8 1

function setToolbarButton(toolbar,buttonMap){
	buttonMap=buttonMap.toString();
  if (buttonMap.substring(0,1)=="1"){//增加
    toolbar.setDisabled(0,false);
  }else{
  	toolbar.setDisabled(0,true);
  }
  
  if (buttonMap.substring(1,2)=="1"){//修改
  ...
  if (buttonMap.substring(2,3)=="1"){//删除
    toolbar.setDisabled(2,false);
  }else{
  	toolbar.setDisabled(2,true);
  }
  

  if (buttonMap.substring(3,4)=="1"){//导入
    toolbar.setDisabled(3,false);
  }else{
  	toolbar.setDisabled(3,true);
  }
  if (buttonMap.substring(4,5)=="1"){//Excel输出
    toolbar.setDisabled(4,false);
  }else{
 ...
 function doExport(){
 	...
 	//获取是否有修改权限,add by linfux
	function isEditAuth(){
       var tab = top.tabpanel.tabs('getSelected');
	   //alert('Selected: '+tab.panel('options').bMap);
	  if(tab){
	     var buttonMap = tab.panel('options').bMap;
	     buttonMap=buttonMap.toString();
		  if (buttonMap.substring(1,2)==="1"){//修改
		     return true;
		  }else{
