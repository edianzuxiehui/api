...
function viewAppParam(apos_id,app_id) {
	...
	var retValue=openDialogFrame(getRootPath()+"/core/aposinfo/AposAppParam-view.jsp",param,"600","450");
}
...
$(function(){
	...
	var param = k.par;
	if(param!=null && param.indexOf('&t=')!=-1) {//存在参数t，不显示APMS信息
		$('.optionalRow').hide();
	}
	$.getJSON("queryAposInfo.do",param,function(data){
		$.each(data.rows,function(idx,item){
			...
			$('#idAPMSFactory').val(item.APMS_FACTORY);
			$('#idAPMSMid').val(item.APMS_MID);
			$('#idAPMSSerialNo').val(item.APMS_SERIALNO);
			...
			$('#idAposAppInfo').datagrid({
				...
				columns:[[
					...
					{title:'商户编号',field:'MERCH_ID',align:'center',width:120,sortable:true,
					...
					{title:'终端号',field:'TERMINAL_ID',align:'center',width:80,sortable:true}
				]]
			});
		});
	});
