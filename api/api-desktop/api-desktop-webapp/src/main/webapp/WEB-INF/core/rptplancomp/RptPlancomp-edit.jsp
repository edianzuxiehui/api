  ...
  <base target="_self">
	...
	<script type="text/javascript" src="<%=basePath%>js/easyui-lang-zh_CN.js"></script>
		...
		<script type="text/javascript">
		  ...
		  if(k.par){
		  	var param=k.par;
			$.getJSON("queryRptPlancomp.do",param,function(data){
				$.each(data.rows,function(idx,item){
			$("#idPlanCode").val($.trim(item.PLAN_CODE));
			$("#idPlanName").val($.trim(item.PLAN_NAME));
			$("#idTaskTotal").val(item.TASK_TOTAL);
			$("#idTaskDone").val(item.TASK_DONE);
			$("#idTaskUndo").val(item.TASK_UNDO);
			$("#idExecuteRate").val(item.EXECUTE_RATE);
				});
            }); 
