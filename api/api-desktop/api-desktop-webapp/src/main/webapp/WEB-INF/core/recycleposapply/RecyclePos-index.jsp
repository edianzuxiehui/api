...
<head>
  <base href="<%=basePath%>">
    ...
    <script type="text/javascript" src="core/recycleposapply/RecyclePos-js.js"></script>
	<script type="text/javascript">
		...
		function queryInfo(){
		       ...
		       var date=new Date();
				var param="flag="+date.getTime()+"&APOS_VALID="+escape(escape($.trim($('#aposValid').val())))+"&REG_ID="+escape(escape($.trim($('#idRegId').val())))+"&REG_ENTIRE_ID="+escape(escape($.trim($('#idRegEntireId').val())))+"&COMPLET_FLAG="+escape(escape($.trim($('#idCompletFlag').val())))+"&MERCH_ID="+escape(escape($.trim($('#idMerchId').val())))+"&TERMINALID="+escape(escape($.trim($('#idTerminalId').val())))+"&MID="+escape(escape($.trim($('#idMidSelect').val())))+"&FID="+escape(escape($.trim($('#idFidSelect').val())))+"&SERIALNO="+escape(escape($.trim($('#idSerialNo').val())));		   
	    ...
	    <div region="north" style="height: 70px;position:relative;"   split="true">  
				...
				<col width="20%" >
							<tr>
								<td>硬件序列号</td>
								<td>
									<input type="text" name="SERIAL_NO" id="idSerialNo" maxlength="30"  style="width: 120px;"/>
								</td>
								<td>分支机构编号</td>
								<td>
							...
							<tr>
								<td>撤机申请状态</td>
								<td>
									<select name="APOS_VALID" id="aposValid" style="width: 100px;">
