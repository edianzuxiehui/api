	...
	<script type="text/javascript">
		...
		function addTaskInfo(){
			$('#ff').form('submit',{
		        url:'addTaskInfo.do',
			    onSubmit:function(){
			    	var aposIds = $('#idAposIds').val();
			    	if(aposIds==''){
			    		msgShow("新增","请先选择应用终端!","info");
			    		return false;
			    	}
			...
			for(var i=0;i<rows.length;i++){
				...
				for(var j=0;j<aposTaskList.length;j++){
					if(rows[i].APOS_ID==aposTaskList[j].aposId){
						rows[i].taskDetail = "<a style='color:blue;' href=\"javascript:taskDetail('"+aposTaskList[j].taskSysId+"','"+aposTaskList[j].aposId+"')\">任务明细</a>";
...
<body class="easyui-layout" fit="true">
	<div region="north" style="height:130px" title="">
...
<fieldset id="modelArea" style="padding:6px;width:95%;border:1px solid #46A3FF; color:#CC3300; line-height:2.0; font-size:12px;" align=center><legend>基本信息</legend>  
	...
	<div region="south" style="height:330px;position:relative;" title="应用终端信息">  
