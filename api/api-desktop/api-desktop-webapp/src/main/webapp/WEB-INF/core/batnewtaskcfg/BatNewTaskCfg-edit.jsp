...
<head>
  ...
  <base target="_self">
	...
	<script type="text/javascript" src="<%=basePath%>core/batnewtaskcfg/BatNewTaskCfg-edit-js.js"></script>
		...
		function addBatNewTaskCfg(){
			...
			if(type ==1){
				...
				var param="flag="+date.getTime()
							+"&QUERY_REG_ID="+$('#idQueryRegId').val()
							+"&QUERY_REG_ENTIRE_ID="+$('#idQueryRegEntireId').val()
							+"&QUERY_APP_ID="+$('#idQueryAppId').val()
							+"&QUERY_APP_VER="+$('#idQueryAppVer').val()
							+"&QUERY_FID="+$('#idFidSelect').val()
							+"&QUERY_MID="+$('#idMidSelect').val()
							+"&QUERY_BEGIN_SERIAL_NO="+$.trim(idQueryBeginSerialNo)
							+"&QUERY_END_SERIAL_NO="+$.trim(idQueryEndSerialNo)+ "&MODE="
							+ $.trim($('#idMode').val()) + "&IP="
							+ $.trim($('#idIP').val()) + "&PORT="
							+ $.trim($('#idPort').val()) + "&MOBILE="
							+ $.trim($('#idMobile').val());
				$('#ff').form('submit',{
			        url:'updateBatNewTaskCfg.do?'+param,
				    onSubmit:function(){
	...
	<div region="north" style="height:180px" title="基本信息">
		...
		<form id="ff" method="post">
				<table class="formTable" style="width:90%;">
					...
					<col  width="100px" class="leftCol"/>
					<col width="250px" >
					<input type="hidden" name="TAB_NO" id="idTabNo" value="">
		...
		<div style="height:100px;"><table id="idTaskDetail"></table></div>
	</div>
				...
				<table class="formTable" style="width:90%;">
					...
					<col width="25%" >
							<tr>
								<td>应用</td>
								<td>
								    <select class="easyui-validatebox" name="QUERY_APP_ID" id="idQueryAppId" style="width:80%" onchange="changeAppVerInfo(this.value)">
									 <option value="">请选择应用</option>
									</select>
								</td>
								<td>应用版本</td>
								<td>
									<select class="easyui-validatebox" name="QUERY_APP_VER" id="idQueryAppVer" style="width:100%">
							...
							<tr>
								<td>厂商</td>
								<td>
									<select  name="QUERY_FID" id="idFidSelect" maxlength="15" style="width:80%" onchange="changeModelByFID(this.value);">
							...
							<tr>
								<td>查询分支机构</td>
								<td>
									<input type="hidden" name="QUERY_REG_ENTIRE_ID" id="idQueryRegEntireId"/>
									<input type="hidden" name="QUERY_REG_ID" id="idQueryRegId" maxlength="8" />
									<input type="text" class="easyui-validatebox" style="background-color: #EEEEEE" readonly name="Query_REG_NAME" id="idQueryRegName" size="21" />
									<input id="idChoose" type="button" class="btn_grid" value="选择" onclick="queryReg('idQueryRegId','idQueryRegName','idQueryRegEntireId');"/>
								</td>
								<td></td><td></td>							
							</tr>
							<tr>
