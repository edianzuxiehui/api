	$(function(){
			...
			{title:'操作',field:'FUNCTION',align:'center',width:140,sortable:true,
			 formatter:function(value,rowData,rowIndex){
			 			var a="";
			 		    //if(rowData.COMPLET_FLAG=="1" && rowData.DEV_ID!=null){
			 				//a='<input type="button" class="btn_grid" value="撤机" onclick="unbindDevId(\''+ rowData.APOS_ID+ '\',\''+rowData.DEV_ID+ '\')"/>';
			 			   a='<input type="button" class="btn_grid" value="撤机" onclick="recyePosPass(\''+ rowData.APOS_ID+ '\',\''+rowData.SERIAL_NO+ '\',\''+rowData.MID+ '\',\'' +rowData.MERCH_ID+ '\',\'' + rowData.TERMINAL_ID +  '\',\''+ rowData.DEV_ID+'\',\'' + rowData.REG_ID +  '\',\'' + rowData.REG_ENTIRE_ID +  '\')"/>';
			 			//}else{
			 				a += '&nbsp;&nbsp;<input type="button" class="btn_grid" value="申请撤销" onclick="recyePosCancle(\''+ rowData.APOS_ID+ '\',\''+rowData.SERIAL_NO+ '\',\''+rowData.MID+ '\',\'' +rowData.MERCH_ID+ '\',\'' + rowData.TERMINAL_ID +  '\',\''+ rowData.DEV_ID+'\',\'' + rowData.REG_ID +  '\',\'' + rowData.REG_ENTIRE_ID +  '\')"/>';
			 			//}
                        return a;  
                    }  
			} 
		...
		function recyePosPass(APOS_ID,SERIAL_NO,MID,MERCH_ID,TERMINAL_ID,DEV_ID,REG_ID,REG_ENTIRE_ID){
			$.messager.confirm('确认', "您确定要撤机吗?", function(r){
				if(r){ 
		...
		function recyePosCancle(APOS_ID,SERIAL_NO,MID,MERCH_ID,TERMINAL_ID,DEV_ID,REG_ID,REG_ENTIRE_ID){
