...
<head>
  ...
  <base target="_self">
    ...
    <script type="text/javascript" src="core/batnewtaskcfg/PlanDef-js.js"></script>
	<script type="text/javascript">
		function queryInfo(){
		    var date=new Date();
			var param="flag="+date.getTime()
				+"&PLANCODE="+escape(escape($.trim($('#idPlanCode').val())))
			...
			$('#idPlanDef').datagrid('options').url = "listPlanDefForBatNewTaskCfg.do?"+param;// 改变它的url  
		...
		function retPlanDef(){
			var rows = $('#idPlanDef').datagrid('getSelections');
		 	if(rows.length==0) {
		 		...
		 		var o = new Object();
		 		o.planCode = rows[0].PLAN_CODE;
		 		if(!rows[0].PLAN_NAME){
		 			rows[0].PLAN_NAME="";
		 		}
		 		o.planName = rows[0].PLAN_NAME+"("+rows[0].PLAN_CODE+")";
